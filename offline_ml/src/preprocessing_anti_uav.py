...
def extract_sequence(seq_path, output_images, output_labels, modality):
    ...
    video_path = seq_path / f"{mapped_modality}.mp4"
    json_path = seq_path / f"{mapped_modality}.json"
    ...
    while cap.isOpened():
        ...
        if frame_idx % SAMPLE_RATE == 0 or exist == 0:
            img_name = f"{seq_path.name}_{modality}_{str(frame_idx).zfill(5)}.jpg"
...
def preprocess():
    ...
    for split in ["train", "val", "test"]:
        ...
        print(f"\n── {split} ──────────────────────────")
        sequences = [d for d in (DATASET_PATH / split).iterdir() if d.is_dir()]
        total_saved = 0
