...
sudo docker run --rm -it --runtime nvidia --network host --privileged -e NVIDIA_DRIVER_CAPABILITIES=all -v /tmp/argus_socket:/tmp/argus_socket -v $(pwd)/offline_ml:/app/offline_ml -v /dev/bus/usb:/dev/bus/usb --device /dev/video0 --device /dev/video2 jetson-si-ml
```
